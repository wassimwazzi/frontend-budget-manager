      ...
      {contributionRange.contributions.map((contribution, index) => (
        ...
        >
          <Form.Label className="mb-2">
            <strong>Goal description:</strong> {contribution.goal.description}
          </Form.Label>
          <Form.Range
            min="0"
            max="100"
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <Transition visible={errors.some((error) => error)} duration={1}>
          <Alert variant="danger" className="mt-4">
            <p>
              Please make sure that the total percentage for each contribution
              range is 100%.
            </p>
          </Alert>
        </Transition>
        <Button type="submit" disabled={errors.some((error) => error)} className="mt-4">
          Submit
        </Button>
        <div className="mt-3">
          <Status
